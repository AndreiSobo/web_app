document.addEventListener('DOMContentLoaded', function () {
    ...
    classifierForm.addEventListener('submit', function (e) {
        ...
        // First try the api/ClassifyPenguin endpoint (which should be the primary endpoint)
        callEndpoint('/api/ClassifyPenguin', payload)
            .then(handleResponse)
            .catch(error => {
                console.error('First endpoint failed:', error);

                // Log extended debug information
                if (debugMode.checked) {
                    outputValue.innerHTML = `
                        <div class="alert alert-warning mb-0">
                            <div class="h5">First endpoint failed, trying fallback...</div>
                            <p>${error.message}</p>
                        </div>
                    `;
                }
                ...
                return callEndpoint('/classify', payload).then(handleResponse);
            })
            .catch(error => {
                ...
                outputValue.innerHTML = `
                    <div class="alert alert-danger mb-0">
                        ...
                        ${debugMode.checked ? `
                            ...
                            <ol>
                                <li>Check if the Azure Function has all required environment variables</li>
                                <li>Ensure the container registry credentials are correct</li>
                                <li>Look at Azure Function logs for more details</li>
                                <li>Try the Debug Endpoint to see server configuration</li>
                            </ol>
                        ` : ''}
                    </div>
                `;
